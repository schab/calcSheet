...
class Interpreter {
   ...
   *     TERM = NUMBER
   *
   * @param  SheetCell The cell to update
  ...
  void interpret(SheetCell cell, boolean edition) {
	...
	Float value = null;
	try { value = (Float)accept(FORMULA); }
  ...
  Object accept(String syntagma) throws InterpreterEvent {
    ...
    if (syntagma.equals(FORMULA)) {
      ...
      try { leftTerm  = (Float)accept(TERM); }
	...
	try {
	  ...
	  try {
	    ...
	    try { rightTerm = (Float)accept(FORMULA); }
	    catch (EndFormula err)       { throw new SyntaxError(); } 
	    catch (SyntagmaMismatch err) { throw new SyntaxError(); } 
	    if (_noEmptyRef) {
	      res = computeOperation( op,
				      res,
				      rightTerm.floatValue() );
	...
	catch (SyntagmaMismatch evt3) { throw new SyntaxError(); }
    ...
    if (syntagma.equals(NUMBER)) {
      ...
      readLeaf();
      try { value = new Float(_leaf); }
      catch (NumberFormatException ex) { throw new SyntagmaMismatch(); }
    ...
    if (syntagma.equals(CELLID)) {
      ...
      int ii=0;
      while ( ii<id.length &&Character.isLetter(id[ii]) )  { ii++; }
      if (ii==0 || ii>=id.length ) throw new SyntagmaMismatch();
      String column = _leaf.substring(0,ii);
      
      
      int jj=ii;
      while ( jj<id.length && Character.isDigit(id[jj]) )  { jj++; }
	...
	throw new SyntaxError();
      ...
      try { value = Float.valueOf(cellVal); }
